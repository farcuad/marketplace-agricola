...
import { auth, getUserProfile, getSellerOrders, updateOrderStatus } from '@/src/lib/firebase';
import type { Order, UserProfile } from '@/src/types';
import DashboardAside from '@/src/components/DashboardAside';
import DashboardHeader from '@/src/components/DashboardHeader';
import { Sprout, Store, CheckCircle, XCircle, Clock } from 'lucide-react';
...
interface SaleCardProps { order: Order; onStatusChange: (id: string, estado: Order['estado']) => void }

function SaleCard({ order, onStatusChange }: SaleCardProps) {
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: 'var(--color-text-muted)' }}>{order.nombre_comprador}</p>
          </div>
...
export default function MisVentasPage() {
  ...
  const loadOrders = useCallback(async (uid: string) => {
    ...
    try { const data = await getSellerOrders(uid); setOrders(data); } finally { setOrdersLoading(false); }
  }, []);

  useEffect(() => { if (user) loadOrders(user.uid); }, [user, loadOrders]);

  const handleStatusChange = async (orderId: string, estado: Order['estado']) => {
    try { await updateOrderStatus(orderId, estado); setOrders((prev) => prev.map((o) => (o.id === orderId ? { ...o, estado } : o))); } catch { Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo actualizar el estado.', confirmButtonColor: '#16a34a' }); }
  };

  const handleLogout = async () => { await signOut(auth); router.replace('/'); };

  const stats = [
    { label: 'Total', value: orders.length, icon: Store },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--color-bg)' }}>
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>Cargando tus ventas...</p>
        </div>
      </div>
    );
  }
        ...
        <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 pb-16">
          <div className="flex justify-between items-center gap-3 flex-wrap gap-y-2 mb-8">
            <h1 className="font-display text-2xl sm:text-3xl font-bold" style={{ color: 'var(--color-text)' }}>
              Mis ventas <Store size={24} className="inline" />
            </h1>
          ...
          <p className="text-sm mb-8 -mt-4" style={{ color: 'var(--color-text-muted)' }}>Historial de ventas generadas desde tus publicaciones</p>
          ...
          <div>
            ...
            {ordersLoading ? (
              ...
              <div className="auth-card p-14 text-center">
                ...
                <p className="mt-4 font-semibold text-lg" style={{ color: 'var(--color-text)' }}>Sin ventas aún</p>
                <p className="mt-1 text-sm" style={{ color: 'var(--color-text-muted)' }}>Cuando un comprador te contacte por WhatsApp, aparecerá aquí</p>
                <Link href="/" className="btn-primary mt-6">Ir al marketplace</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {orders.map((order) => (<SaleCard key={order.id} order={order} onStatusChange={handleStatusChange} />))}
              </div>
            )}
          </div>
        </main>
